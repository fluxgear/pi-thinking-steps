...
import { deriveThinkingSteps, parseThinkingMode, summarizeThinkingText } from "../parse.js";
...
describe("deriveThinkingSteps", () => {
	...
	it("creates a faithful fallback summary for redacted reasoning", () => {
		...
		assert.equal(steps[0]?.summary, "Reasoning hidden by provider");
	});
});
...
describe("renderThinkingStepsLines", () => {
	...
	it("expanded mode renders full step details with connected pipes", () => {
		...
		assert.match(joined, /thinking steps · expanded/);
		assert.match(joined, /├─ ⌕ Inspect the current renderer implementation/);
		assert.match(joined, /│  Inspect the current renderer implementation\./);
		assert.match(joined, /└─ ✓ Verify the refresh path after mode changes/);
